...
digit_classifer, classifier_funs = initialise_network(one_dim_length, [256, 32], length(digits), 
...
trained_net, training_df = train(digit_classifer, classifier_funs,
                                ...
                                n_epochs = 100, scheduler = false, ηᵢ = 0.005)

# Plot loss
using Plots

plot(training_df.epoch, log10.(training_df.loss), alpha = 1/2, legend = :topright, 
     xlabel = "Epoch", ylabel = "cross-entropy loss", label = "Training Loss", title = "MNIST Classification Model")
